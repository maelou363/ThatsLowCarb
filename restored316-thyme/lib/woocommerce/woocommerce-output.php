 ...
 * Thyme Theme.
 *
 * This file adds the WooCommerce styles and the Customizer additions for WooCommerce to the Thyme Theme.
 *
 * @package Thyme Theme
...
add_filter( 'woocommerce_enqueue_styles', 'thyme_woocommerce_styles' );
/**
 ...
 */
function thyme_woocommerce_styles( $enqueue_styles ) {

	$enqueue_styles[ genesis_get_theme_handle() . '-woocommerce-styles' ] = [
		'src'     => CHILD_URL . '/lib/woocommerce/thyme-woocommerce.css',
		'deps'    => '',
...
add_action( 'wp_enqueue_scripts', 'thyme_woocommerce_css' );
/**
 ...
 */
function thyme_woocommerce_css() {
	...
	$woo_css .= ( thyme_get_default_link_color() !== $color_link ) ? sprintf(
		'
	...
	$woo_css .= ( thyme_get_default_button_color() !== $color_button ) ? sprintf(
		'
