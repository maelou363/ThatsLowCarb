 ...
 * Thyme Theme.
 *
 * This file adds the required WooCommerce setup functions to the Thyme Theme.
 *
 * @package Thyme Theme
...
add_filter( 'woocommerce_style_smallscreen_breakpoint', 'thyme_woocommerce_breakpoint' );
/**
 ...
 */
function thyme_woocommerce_breakpoint() {
...
add_filter( 'genesiswooc_products_per_page', 'thyme_default_products_per_page' );
/**
 ...
 */
function thyme_default_products_per_page() {

	return 8;

}

add_filter( 'woocommerce_pagination_args', 'thyme_woocommerce_pagination' );
/**
 ...
 */
function thyme_woocommerce_pagination( $args ) {

	$args['prev_text'] = sprintf( '&laquo; %s', __( 'Previous Page', 'thyme-pro' ) );
	$args['next_text'] = sprintf( '%s &raquo;', __( 'Next Page', 'thyme-pro' ) );

	return $args;

}

add_action( 'after_switch_theme', 'thyme_woocommerce_image_dimensions_after_theme_setup', 1 );
/**
 * Defines WooCommerce image sizes on theme activation.
 *
 * @since 1.0.0
 */
function thyme_woocommerce_image_dimensions_after_theme_setup() {
	...
	thyme_update_woocommerce_image_dimensions();

}

add_action( 'activated_plugin', 'thyme_woocommerce_image_dimensions_after_woo_activation', 10, 2 );
/**
 ...
 */
function thyme_woocommerce_image_dimensions_after_woo_activation( $plugin ) {
 ...
 * @since 1.0.0
 */
function thyme_update_woocommerce_image_dimensions() {
...
add_filter( 'woocommerce_get_image_size_gallery_thumbnail', 'thyme_gallery_image_thumbnail' );
/**
 ...
 */
function thyme_gallery_image_thumbnail( $size ) {
